...
for current in path:
    if basename(current) == "interface":
        ...
        UIpath = join(join(current, "qt"), "ui")                                 # Imagen para la plantilla UI
        stylePath = join(join(join(current, "qt"), "stylesheet"), "MainWindow")  # Path de los qss
        productPath = join(join(current, "images"), "inventory")                 # Path de las imágenes de productos
        break
